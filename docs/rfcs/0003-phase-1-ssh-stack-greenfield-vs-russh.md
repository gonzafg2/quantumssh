...
The two candidate foundations score very differently against those commitments. The summary table below comes from the project's internal Phase-1 decision notes §"Decisión 1 — `russh` vs. implementación greenfield"; the full evidence with citations lives there.
...
- **Workspace topology.** Two crates, flat layout (`crates/quantumssh` binary + `crates/quantumssh-core` library), per the project's internal Phase-1 decision notes §"Decisión 2".
...
- **`RustCrypto/ml-kem` 0.3.0** as the ML-KEM-768 implementation, with `libcrux-ml-kem` named as fallback under specific triggers (formal verification requirement, OpenSSH byte-parity requirement) per the project's internal Phase-1 decision notes §"Decisión 3".
- **CI interop gate** (Debian trixie container, OpenSSH 10.0p1) per the project's internal Phase-1 decision notes §"Decisión 5".
