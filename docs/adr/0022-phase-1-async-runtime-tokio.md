...
- **Sequential accept loop in Phase 1, spawn-and-join shape.** `loop { let (sock, peer) = listener.accept().await?; if let Err(join_err) = tokio::spawn(handle(sock, peer)).await { /* emit connection.closed with reason = join_err */ } }`. The immediate `.await` on the `JoinHandle` keeps effective concurrency at one, while the `spawn` (a) makes the compiler enforce `Send` on the per-connection future from the first crate and (b) isolates a per-connection panic from the accept loop: the `JoinError` **must be captured and logged** as `connection.closed` with the panic reason — not discarded with `let _ =` — so a crashing handler leaves an audit trace instead of taking the server down silently. Moving to concurrent connections in Phase 2 is *removing* the immediate join (track the handle instead), not a refactor.
- **Handshake budget: 30 seconds, configurable, default documented.** Threat model §5.1.3 requires handshake-completion deadlines to be configurable with documented defaults. Phase 1 enforces a single deadline of **30 seconds from TCP accept to userauth completion** via `tokio::time::timeout`, exposed as a `--handshake-timeout` CLI flag whose default appears in `--help`. §5.1.3's per-source rate limits and half-open caps are properties of a concurrent accept loop and land with Phase 2 (under the sequential loop the effective half-open cap is one).
