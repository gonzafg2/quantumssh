...
- The `quantumssh` binary is the **only** place a subscriber is constructed. All log output — the audit tier included — is written to **stderr**, the sink threat-model §2.7 names (stdout is reserved and unused by logging). Phase 1 selects the format at startup by examining the stream it actually writes to: **JSON when stderr is not a TTY** (files, pipes, journald — the shipping case of §5.5.1 and §6.2), human-readable when stderr is a TTY (interactive development), overridable either way with `--log-format {json,human}`. Keying the detection on stdout would silently break the guarantee under `quantumssh 2> file` with an attached terminal.
- **The audit tier is not operator-suppressible.** The §2.7-mandated events (`auth.succeeded`, `auth.failed`, `exec.started`, `exec.finished`, plus `kex.failed`) are emitted under a dedicated `audit` target and flow through their **own subscriber layer whose filter is compiled in and never consults the environment**; `RUST_LOG`/`EnvFilter` governs only the separate general layer. This is deliberately two layers, not one `EnvFilter` with a "floor" directive: `EnvFilter` resolves competing directives by specificity, so a single-filter design would let `RUST_LOG=audit=off` (more specific than any floor) silently drop the entire mandated trail. With the two-layer design there is no environment directive that reaches the audit layer at all. `RUST_LOG` quiets noise; disabling audit events is not a supported configuration in Phase 1.
...
- Structured fields (not string interpolation) make the logs JSON-shippable, and in the JSON format immune to the terminal-escape log-injection of §5.4.3 (serde escapes control bytes); they align with the one-way-sink posture of §5.5.1. The §5.4.3 defence holds in the default deployment because **JSON is the default whenever stderr (the log stream) is not a TTY** — a log file or pipeline never receives raw control bytes. The human-readable formatter (TTY default) additionally **escapes control bytes in string field values** (`\xNN` visible form) via a small custom field formatter, so an attacker-chosen `command` cannot replay ANSI sequences into an operator's live terminal either; stock `tracing_subscriber::fmt()` does not do this, which is why the field formatter is part of this decision and not an implementation afterthought.
...
- Implementation: M4 ([#78](https://github.com/gonzafg2/quantumssh/pull/78)) and M5 ([#84](https://github.com/gonzafg2/quantumssh/pull/84)) — `tracing` events throughout `quantumssh-core` (`auth.*` in `src/auth.rs`, `exec.*` in `src/exec.rs`), `tracing-subscriber` initialisation in `crates/quantumssh/src/main.rs`, and the escape-safe field formatter in `crates/quantumssh/src/log_fields.rs`.
