...
- The `quantumssh` binary is the **only** place a subscriber is constructed. Phase 1 selects the format at startup: **JSON when stdout is not a TTY** (the shipping/pipeline case threat-model §5.5.1 and §6.2 describe), human-readable when it is (interactive development), overridable either way with `--log-format {json,human}`.
