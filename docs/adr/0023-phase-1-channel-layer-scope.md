...
- **`exec` is the only honoured request.** Its `command` string is passed to the child process spawn described in ADR-0022 (`std::process::Command` on `spawn_blocking`). Under threat-model §8.12 the child runs as the QuantumSSH service-account UID, not a per-user UID — Phase 1 is single-user by design.
...
- **One channel per connection.** A second `SSH_MSG_CHANNEL_OPEN` while a session channel is live is answered with `SSH_MSG_CHANNEL_OPEN_FAILURE`. SSH-2 permits multiplexing; Phase 1 deliberately does not. This collapses a whole class of channel-id bookkeeping and concurrent-channel state out of the walking skeleton.
...
- **Flow control is real, not stubbed.** The window mechanism (`SSH_MSG_CHANNEL_WINDOW_ADJUST`) is implemented correctly in both directions; a stubbed/ignored window is a correctness bug an OpenSSH client surfaces under load. Phase 1 initial window: **2 MiB**; maximum packet size: **32 KiB** (matching OpenSSH's defaults, so the interop client's assumptions hold).
