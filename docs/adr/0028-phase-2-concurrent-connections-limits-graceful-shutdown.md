...
- **Admission control, checked before spawn.** A connection counts as **half-open** from TCP accept until userauth success (the same span the handshake budget covers). Four checks, evaluated in this order — the three non-mutating cap reads first and the state-mutating rate-limit check last, so a connection refused by a cap never touches or grows the per-source map (the load-shedding order for the rotating-source flood the Consequences disclose); the first check that fails names the structured `reason` on the refusal event:
  ...
  4. **Per-source accept rate limit** — the token bucket defined in the next bullet. Tokens are consumed only by connections that passed the three caps, so a refusal by a cap never charges the source's bucket.
...
- Implementation: TBD — `crates/quantumssh-core/src/server.rs` (accept loop, admission control, shutdown drain), `crates/quantumssh-core/src/channel.rs` and `crates/quantumssh-core/src/exec.rs` (cooperative shutdown, the `exec.finished` drop-guard), both `Cargo.toml`s (feature delta), the runtime builder in `crates/quantumssh/src/main.rs` (blocking-pool sizing). Paths exist today; the concurrent shapes do not.
