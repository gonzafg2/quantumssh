...
- **Task tracking: `tokio::task::JoinSet`.** The accept loop `tokio::select!`s over `accept()`, `JoinSet::join_next()`, and the shutdown signal. Every task completion is reaped; a `JoinError` (panic) is logged as `connection.closed` with the panic reason, preserving ADR-0022's panic-isolation and audit-trace guarantees. No detached tasks: every connection the server holds is in the set.
- **Admission control, checked before spawn.** A connection counts as **half-open** from TCP accept until userauth success (the same span the handshake budget covers). Three limits, evaluated in this order:
  ...
  A connection over any cap is closed immediately after accept — no version banner is sent — and logged as a new **`connection.refused`** event carrying `peer_addr` and which limit was hit. Counting is by RAII guard tied to the connection task, so a panicking handler releases its slots.
