...
**Cero legacy.** Sin SSH-1. Sin RSA-1024. Sin DSA. Sin modos CBC. Sin `diffie-hellman-group1-sha1`. Sin autenticación por contraseña en el perfil por defecto. Nos rehusamos a heredar 25 años de *"sigue ahí porque el router de alguien lo necesita"*. Y "legacy" no es solo esta lista fija: es todo primitivo criptográfico que NIST o IETF haya **prohibido** (*disallowed*). La deprecación activa la migración gestionada; la prohibición marca la línea de legacy. Esa lista es el piso permanente; la frontera avanza al ritmo de los estándares, y migramos antes de esas fechas. El procedimiento está en [RFC-0007](docs/rfcs/0007-cryptographic-primitive-migration-procedure.md).
